...
import { AttachmentTypeSortColumn } from '../../../core/api/attachment-types.api';
import { ToastService } from '../../../shared/ui/toast';
import {DEFAULT_PAGE_NUMBER, DEFAULT_PAGE_SIZE} from '../../../core/constant/constant';

@Component({
  selector: 'app-attachment-types-list-page',
  standalone: true,
  imports: [
    CommonModule,
    ButtonComponent,
  ...
  template: `
    <div class="p-4  relative">
      ...
      <app-card>
        <app-attachment-type-list
          [items]="store.attachmentTypePage().elements"
          (view)="goView($event)"
          (edit)="goEdit($event)"
          (remove)="remove($event)"
          (sort)="onSort($event)"
        />
        <app-paginator
          [total]="store.attachmentTypePage().totalElements"
          [page]="uiPage()"
          [pageSize]="uiPageSize()"
...
export class AttachmentTypesListPage {
  ...
  uiPage = computed(() => (this.store.attachmentTypePage().page ?? 0) + 1);
  uiPageSize = computed(() => this.store.attachmentTypePage().size ?? 10);

  sortColumn: AttachmentTypeSortColumn = 'name';
  ...
  async onSort(e: { key: string; direction: DirectionType }) {
    ...
    this.sortColumn = e.key as AttachmentTypeSortColumn;
  ...
  goEdit(id: string) {
    ...
    this.router.navigate(['/attachment-types', id, 'edit']);
  }

  goView(id: string) {
    // navigate to an absolute feature path to avoid relative routing issues causing NG04002
    this.router.navigate(['/attachment-types', id]);
  }
  ...
  async remove(id: string) {
    const ok = await this.svc.open({ message: this.i18n.t('confirm.delete.attachmentType') });
    if (!ok) return;
