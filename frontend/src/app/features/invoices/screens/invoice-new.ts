...
export class InvoiceNewPage {
  ...
  lineModel = signal<InvoiceLineFormValue>(this.initialInvoiceLine);
  ...
  openLineDialog() {
    ...
    // reset the model each time
    this.lineModel.set(this.initialInvoiceLine);
  }
  ...
  closeLineDialog() {
    this.lineDialogOpen.set(false);
  }
  onLineDialogClosed() {
    this.editingIndex.set(null);
    this.lineModel.set(this.initialInvoiceLine);
  }
  onLineFormChange(v: InvoiceLineFormValue) {
    this.lineModel.set(v);
  }
  private addLineCommon(closeAfter: boolean) {
    ...
    const payload = {
      ...
      unitPriceCurrency: m.unitPriceCurrency ?? null,
      amount: m.amount ?? '0',
      amountCurrency: m.amountCurrency ?? (m.unitPriceCurrency ?? null)
    } as any;
