      ...
      <app-card>
        <div class="h-12 flex items-center justify-between">
          ...
          <div class="inline-flex items-center gap-2">
            <app-button size="sm" variant="secondary" shadow="md" hoverShadow="base" [disabled]="disableNew()" (clicked)="newClicked.emit()">
              <span class="material-symbols-outlined text-base">add</span>
              <span class="hidden sm:inline">{{ 'actions.new' | t }}</span>
            </app-button>
            <app-button size="sm" variant="secondary" shadow="md" hoverShadow="base" [disabled]="disableEdit()" (clicked)="editClicked.emit()">
              <span class="material-symbols-outlined text-base">edit</span>
              <span class="hidden sm:inline">{{ 'actions.edit' | t }}</span>
            </app-button>
            <app-button size="sm" variant="ghost" shadow="md" hoverShadow="base" [disabled]="disableCancel()" (clicked)="cancelClicked.emit()">
              <span class="material-symbols-outlined text-base">close</span>
              <span class="hidden sm:inline">{{ 'actions.cancel' | t }}</span>
            </app-button>
            <app-button size="sm" variant="primary" shadow="md" hoverShadow="base" [disabled]="disableSave()" (clicked)="saveClicked.emit()">
