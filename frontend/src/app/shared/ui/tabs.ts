      ...
      <div class="flex items-center gap-2 border-b border-token">
        @for (t of items(); track t.id) {
          ...
          >
            <span>{{ t.label }}</span>
            @if (t.badge != null) {
              <span class="ml-2 inline-flex items-center justify-center min-w-5 h-5 px-1  text-[10px] bg-neutral-200 text-neutral-800 dark:bg-neutral-700 dark:text-neutral-100">{{ t.badge }}</span>
            }
          </button>
        }
      </div>
